...
//    @Mock private RoomDeviceCacheRepository cacheRepository;
    ...
    void getRoomDevices_CacheHit() {
        List<ExternalRoomDeviceInfo> apiList = List.of(new ExternalRoomDeviceInfo(ROOM_ID, "eui", "name", null));
//        when(cacheRepository.get(1L)).thenReturn(cachedList);
        when(roomDeviceClient.getRoomDevices(ROOM_ID)).thenReturn(apiList);

        List<ExternalRoomDeviceInfo> result = roomDeviceCacheService.getRoomDevices(ROOM_ID);

        assertThat(result).isEqualTo(apiList);
        verify(roomDeviceClient, times(1)).getRoomDevices(ROOM_ID);
    }
    ...
    void getRoomDevices_ClientFail_ReturnsDummyData() {
        ...
        when(roomDeviceClient.getRoomDevices(ROOM_ID)).thenThrow(new RuntimeException("External API Connection Refused"));
