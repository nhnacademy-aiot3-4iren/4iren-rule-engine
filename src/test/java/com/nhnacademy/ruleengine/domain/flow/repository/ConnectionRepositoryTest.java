...
class ConnectionRepositoryTest {
    ...
    void setUp() {
        ...
        conn1 = connectionRepository.save(Connection.builder()
                .flow(flow1).sourceNode(node1).targetNode(node2).branchType("TRUE").build());
    ...
    @Test
    @DisplayName("findAllByFlowId - 해당 플로우 커넥션만 반환")
    void findAllByFlowId_returnsConnectionsOfFlow() {
        List<Connection> result = connectionRepository.findAllByFlowId(flow1.getId());
    ...
    @Test
    @DisplayName("deleteAllByFlowId - 해당 플로우 커넥션 삭제")
    void deleteAllByFlowId_deletesConnectionsOfFlow() {
        connectionRepository.deleteAllByFlowId(flow1.getId());

        List<Connection> result1 = connectionRepository.findAllByFlowId(flow1.getId());
        assertThat(result1).isEmpty();

        List<Connection> result2 = connectionRepository.findAllByFlowId(flow2.getId());
        assertThat(result2).hasSize(1);
    }


    @Test
    @DisplayName("findAllByFlowIdIn - 여러 플로우 커넥션 한번에 조회")
    void findAllByFlowIdIn_returnsAllConnections() {
        List<Connection> result = connectionRepository.findAllByFlowIdIn(
    ...
    @Test
    @DisplayName("findAllByFlowIdIn - 빈 리스트 입력 시 빈 결과")
    void findAllByFlowIdIn_emptyInput_returnsEmpty() {
        List<Connection> result = connectionRepository.findAllByFlowIdIn(List.of());
