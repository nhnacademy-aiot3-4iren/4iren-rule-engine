...
public class FlowValidator {
    private final NodeConfigValidatorRegistry nodeConfigValidatorRegistry;

    // 센서 메타 정보 없이 플로우의 기본 구조만 검증한다.
    public void validate(@NotEmpty List<NodeInfo> nodes, @NotNull List<ConnectionInfo> connections){
        validate(nodes, connections, List.of());
    }
    ...
    public void validate(
        ...
        Map<Long, NodeInfo> nodeMap = nodes.stream()
    ...
    private void validateNodeCount(@NotEmpty List<NodeInfo> nodes, List<ValidationErrorResponse.ValidationError> errors){
    ...
    private void validateRequiredNodeTypes(List<NodeInfo> nodes, List<ValidationErrorResponse.ValidationError> errors) {
    ...
    private void validateNodeIds(List<NodeInfo> nodes, List<ValidationErrorResponse.ValidationError> errors) {
    ...
    // 각 노드의 nodeConfig 필수값과 nodeType 일치 여부, 타입별 설정 유효성을 검증한다.
    ...
    private void validatePortRules(
        ...
        Map<Long, NodeType> nodeTypeById = nodes.stream()
                .filter(node -> node.nodeId() != null)
                .collect(Collectors.toMap(NodeInfo::nodeId, NodeInfo::nodeType, (first, second) -> first));
    ...
    private void validateSingleEntryPoint(List<NodeInfo> nodes, List<ConnectionInfo> connections, List<ValidationErrorResponse.ValidationError> errors) {
        ...
        if(startNodeCount == 0){
            ...
            errors.add(ValidationErrorResponse.ValidationError.of("nodes", "입력이 없는 시작 노드는 1개여야 합니다. 현재: " + startNodeCount + "개"));
        }
    }

    // 연결 그래프를 DFS로 순회해 순환 연결이 존재하는지 검증한다.
    private void validateNoCycle( List<NodeInfo> nodes, List<ConnectionInfo> connections, List<ValidationErrorResponse.ValidationError> errors) {
    ...
    private Map<Long, Degree> calculateDegrees(List<NodeInfo> nodes, List<ConnectionInfo> connections) {
